...
function sender(mode: string) {
  let fetchCalls = 0;
  let credentialCalls = 0;
  let healthWrites = 0;
  const send = createWhatsAppCloudSender({
    resolveSendMode: () => resolveWhatsAppSendMode({ META_WHATSAPP_SEND_MODE: mode }),
    evaluateCapability: evaluateMessagingCapability,
    getCredential: async () => {
      credentialCalls += 1;
      return { externalId: '1022209807648757', accessToken: 'server-secret' };
    },
