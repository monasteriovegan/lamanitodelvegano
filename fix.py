...
# Enhance Nuestra historia
old_hist = '''<p>La Manito Del Vegano nació de una convicción profunda: que la alimentación plant based puede ser deliciosa, accesible y llena de amor. Desde nuestra cocina en Santiago, elaboramos cada producto con ingredientes 100% vegetales.</p>
...
new_hist = '''<p><strong>La Manito Del Vegano</strong> nació de una convicción profunda: que la alimentación <em>plant based</em> puede ser deliciosa, accesible y llena de amor. Todo comenzó en nuestra cocina, buscando alternativas éticas que no sacrificaran el sabor de la comida tradicional chilena.</p>
...
idx = js.find('function renderAdminTab(tab) {')
if idx != -1:
    js = js[:idx]

new_js = """function renderAdminTab(tab) {
