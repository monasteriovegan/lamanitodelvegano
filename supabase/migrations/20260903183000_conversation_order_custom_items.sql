-- Conversational sales may include explicit custom products (for example a
-- made-to-order cake) that are not part of the stock catalog. Catalog items
-- remain validated and stock-controlled; custom items must have no productoId,
-- must be flagged custom=true, and must carry an explicit name/price.
...
begin
  ...
  if jsonb_typeof(p_stock_items) <> 'array' then
    raise exception 'invalid_conversation_stock_items';
  end if;
  ...
  loop
    ...
    if item_quantity <= 0 or item_price < 0 then
      raise exception 'invalid_conversation_order_item_values';
    end if;
    ...
    if raw_product_id is null then
      if coalesce((item ->> 'custom')::boolean, false) is not true
        ...
        raise exception 'invalid_custom_conversation_order_item';
      end if;
    else
      begin
        product_id := raw_product_id::uuid;
      exception when others then
        raise exception 'invalid_conversation_order_product_id';
      end;
      ...
      if not exists (
        select 1
          from jsonb_array_elements(p_stock_items) s
  ...
  loop
    begin
      ...
      item_price := (item ->> 'precio')::numeric;
    exception when others then
      raise exception 'invalid_conversation_stock_item';
    end;
