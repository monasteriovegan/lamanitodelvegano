...
do $$
declare required_table text; required_column text; actual_type text;
begin
  ...
  if actual_type is distinct from 'uuid' then raise exception 'omnichannel_preflight_type_mismatch:businesses.id:%',coalesce(actual_type,'missing'); end if;
  ...
  if actual_type is distinct from 'uuid' then raise exception 'omnichannel_preflight_type_mismatch:customers.id:%',coalesce(actual_type,'missing'); end if;
  ...
  if actual_type is distinct from 'uuid' then raise exception 'omnichannel_preflight_type_mismatch:orders.id:%',coalesce(actual_type,'missing'); end if;
  ...
  if actual_type is distinct from 'text' then raise exception 'omnichannel_preflight_type_mismatch:productos.id:%',coalesce(actual_type,'missing'); end if;
  foreach required_column in array array['business_id','phone','email','nombre','crm_status'] loop
    if not exists(select 1 from information_schema.columns where table_schema='public' and table_name='customers' and column_name=required_column) then
      raise exception 'omnichannel_preflight_missing_column:customers.%',required_column;
    end if;
  end loop;
  if to_regclass('public.crm_conversations') is not null then
    foreach required_column in array array['id','business_id','customer_id','channel','external_thread_id','status','last_message_at','updated_at'] loop
      if not exists(select 1 from information_schema.columns where table_schema='public' and table_name='crm_conversations' and column_name=required_column) then
        raise exception 'omnichannel_preflight_missing_column:crm_conversations.%',required_column;
      end if;
    end loop;
  end if;
  if to_regclass('public.crm_messages') is not null then
    foreach required_column in array array['id','conversation_id','direction','sender_type','text','message_type','raw_payload','sent_at','external_message_id'] loop
      if not exists(select 1 from information_schema.columns where table_schema='public' and table_name='crm_messages' and column_name=required_column) then
        raise exception 'omnichannel_preflight_missing_column:crm_messages.%',required_column;
      end if;
    end loop;
  end if;
...
do $$ begin
  if exists(select 1 from crm_conversations group by business_id,channel,external_thread_id having count(*)>1) then
    raise exception 'omnichannel_preflight_duplicate_conversations';
  end if;
end $$;
create unique index if not exists crm_conversations_business_channel_thread_uidx on crm_conversations(business_id,channel,external_thread_id);
