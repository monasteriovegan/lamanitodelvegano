...
const DETAILS: Record<string, { icon: string; title: string; description: string }> = {
  ...
  remy: { icon: '🤖', title: 'Remy', description: 'Ventas y atención automática. Puede usar un modelo distinto de Wonka.' },
};
...
export default async function AgentesPage() {
  ...
  return <div className="max-w-4xl">
    ...
    <p className="mb-5 text-xs leading-5 text-white/45">Cada agente conserva su propio modelo. Wonka no cambia automáticamente entre Gemini, GPT o Claude. Las versiones web de otros LLM son herramientas externas y solo se usan cuando tú lo ordenas.</p>
          ...
          <form action={saveAgentRuntime} className="space-y-4">
            ...
            <div>
              ...
              <select name="provider" defaultValue={row.provider || 'gemini'} className="w-full rounded-xl border border-white/10 bg-[#07130e] px-3 py-2.5 text-sm text-white">
                <option value="gemini">Gemini API · conectado</option>
              </select>
              <p className="mt-1 text-[10px] text-white/30">GPT/Claude API aparecerán aquí cuando conectemos esas credenciales. No se seleccionan solos.</p>
            </div>
            <div>
              <label className="mb-1.5 block text-[10px] font-semibold uppercase tracking-wider text-white/40">Modelo</label>
              <input name="model" defaultValue={row.model || 'gemini-2.5-flash'} className="w-full rounded-xl border border-white/10 bg-white/[0.035] px-3 py-2.5 text-sm text-white" />
            </div>
