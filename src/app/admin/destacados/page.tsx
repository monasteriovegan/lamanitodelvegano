...
export default async function AdminDestacadosPage() {
  ...
  return (
    <div>
      <PageHeader
        eyebrow={`${destacados.length} de ${productos.length} productos activos`}
        title="⭐ Destacados"
      />
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <SectionCard title={`En portada (${destacados.length})`}>
          {destacados.length === 0 ? (
            <EmptyState emoji="⭐" texto="Ningún producto destacado todavía." />
          ) : (
            <div className="flex flex-col gap-2">
              {destacados.map((p) => (
                <FilaProducto key={p.id} producto={p} destacado />
              ))}
            </div>
          )}
        </SectionCard>
        ...
        <SectionCard title={`Resto del catálogo (${resto.length})`}>
          {resto.length === 0 ? (
            <EmptyState emoji="🌿" texto="Todo tu catálogo ya está destacado." />
          ) : (
            <div className="flex flex-col gap-2 max-h-[520px] overflow-y-auto pr-1">
              {resto.map((p) => (
                <FilaProducto key={p.id} producto={p} destacado={false} />
              ))}
            </div>
          )}
        </SectionCard>
    ...
    <div className="flex items-center justify-between gap-3 bg-white/[0.02] rounded-xl px-4 py-2.5 border border-white/5">
      ...
      <form action={toggleDestacado.bind(null, producto.id, producto.destacado)} className="shrink-0">
        <button
          ...
          className={
            destacado
              ? 'text-xs font-semibold px-3 py-1.5 rounded-full bg-[rgba(212,175,55,0.15)] text-gold border border-[rgba(212,175,55,0.3)] hover:bg-[rgba(212,175,55,0.25)] transition-colors'
              : 'text-xs font-semibold px-3 py-1.5 rounded-full bg-white/5 text-muted border border-white/10 hover:border-[rgba(0,255,179,0.3)] hover:text-neon transition-colors'
          }
        >
