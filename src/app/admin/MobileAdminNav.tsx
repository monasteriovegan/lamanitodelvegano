...
export function MobileAdminNav() {
  ...
  useEffect(() => {
    ...
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.register('/wonka-sw.js', { scope: '/' }).catch(() => undefined);
    }
        ...
        <div className="fixed inset-0 z-[780] md:hidden">
          ...
          <section className="absolute inset-x-3 bottom-[86px] max-h-[72dvh] overflow-y-auto overscroll-contain rounded-3xl border border-white/10 bg-[#06110c] p-3 shadow-2xl">
            <div className="flex items-center justify-between px-2 py-2">
              <div>
                <div className="text-[10px] uppercase tracking-[0.18em] text-neon/65">Synthetiq Panel Maestro</div>
                <div className="mt-1 text-base font-black text-white">Más herramientas</div>
              </div>
              <button onClick={() => setOpen(false)} className="h-10 w-10 rounded-full border border-white/10 text-lg text-white/60" aria-label="Cerrar menú">×</button>
            </div>

            {!standalone && (
              <div className="mb-3 rounded-2xl border border-neon/25 bg-neon/[0.07] p-3">
                <button onClick={() => void installApp()} className="flex min-h-12 w-full items-center gap-3 text-left">
                  <span className="text-2xl">📲</span>
                  <span className="min-w-0">
                    <span className="block text-sm font-black text-neon">Instalar Panel Maestro</span>
                    <span className="mt-0.5 block text-[10px] leading-4 text-white/45">Abrirlo como app desde la pantalla de inicio</span>
                  </span>
                </button>
                {installHelp && <p className="mt-2 border-t border-white/8 pt-2 text-[10px] leading-4 text-white/55">{installHelp}</p>}
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-2">
              {more.map((item) => (
                <Link
                  ...
                  className={`flex min-h-12 items-center rounded-2xl border px-3 py-3 text-sm font-semibold ${active(pathname, item.href) ? 'border-neon/35 bg-neon/12 text-neon' : 'border-white/8 bg-white/[0.03] text-white/70'}`}
                >
      ...
      <nav className="fixed inset-x-0 bottom-0 z-[800] md:hidden border-t border-white/10 bg-[#04100b]/95 backdrop-blur-xl px-2 pt-1.5 pb-[max(6px,env(safe-area-inset-bottom))] shadow-[0_-12px_35px_rgba(0,0,0,0.45)]">
        <div className="grid grid-cols-5 gap-1">
          {primary.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`flex min-h-[54px] flex-col items-center justify-center rounded-2xl text-[10px] font-bold ${isActive ? 'bg-neon/12 text-neon' : 'text-white/55'}`}>
                <span className="text-lg leading-none">{item.icon}</span>
                <span className="mt-1">{item.label}</span>
              </Link>
            );
          })}
          <button onClick={() => setOpen((value) => !value)} className={`flex min-h-[54px] flex-col items-center justify-center rounded-2xl text-[10px] font-bold ${open ? 'bg-neon/12 text-neon' : 'text-white/55'}`}>
