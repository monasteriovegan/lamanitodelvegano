...
import { INSTAGRAM_LOGIN_SCOPES, instagramLoginAuthorizationUrl } from '@/lib/meta/instagram-login-oauth';
...
export async function GET(request: Request) {
  ...
  const { error } = await db.from('meta_oauth_states').insert({
    ...
    requested_capabilities: [...INSTAGRAM_LOGIN_SCOPES],
    expires_at: new Date(Date.now() + 10 * 60_000).toISOString(),
  });
  if (error) return Response.json({ error: 'oauth_state_create_failed' }, { status: 500 });

  return Response.redirect(instagramLoginAuthorizationUrl(url.origin, state));
}
