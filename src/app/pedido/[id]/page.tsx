...
export default async function PedidoConfirmacionPage({
  ...
  const trackingCode = pedido.tracking_number || `LMV-${String(pedido.id).padStart(6, '0')}`;

  return (
    <SiteShell>
      {esExito && <PurchaseTracking pedidoId={String(pedido.id)} total={Number(pedido.total || 0)} />}
      <main className="pt-[100px] px-4 pb-16 max-w-[480px] mx-auto text-center">
        ...
        <div className="glass rounded-2xl p-5 mb-6 text-left">
          ...
          <p className="font-mono font-bold text-lg text-neon mb-3">{trackingCode}</p>
          <p className="text-sm text-neon font-bold">Total: ${Number(pedido.total || 0).toLocaleString('es-CL')}</p>
        </div>

        <div className="flex flex-col gap-2">
          <Link
            href={`/seguimiento?tracking=${encodeURIComponent(trackingCode)}`}
