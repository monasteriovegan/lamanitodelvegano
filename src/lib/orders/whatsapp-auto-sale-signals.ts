...
// enough to be worth a batched extraction call.
...
export function hasReceiptBackedWhatsappConfirmation(messages: WhatsappMessageRow[]) {
  ...
  for (const message of messages) {
    ...
    if (!receiptSeen || !isHumanWhatsappEcho(message)) continue;
...
export function hasBusinessPaymentConfirmation(messages: WhatsappMessageRow[]) {
  ...
  }) || hasReceiptBackedWhatsappConfirmation(messages);
}
