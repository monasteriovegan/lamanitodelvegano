...
function notificationText(order: AdminOrder, event: OrderNotificationEvent) {
  ...
  return `${hello}¡Tu pedido #${order.numeric_id} va en camino! 🚚${tracking} Puedes revisar su estado en https://lamanitodelvegano.vercel.app/pedido/${order.numeric_id}`;
}
...
function notificationEmail(order: AdminOrder, event: OrderNotificationEvent) {
  ...
  const content = event === 'payment_paid'
    ...
    : `<h1 style="font-size:22px;margin:0 0 10px;">Tu pedido va en camino 🚚</h1>
       ...
       ${tracking}
       <p style="margin:18px 0 0;"><a href="https://lamanitodelvegano.vercel.app/pedido/${orderId}" style="display:inline-block;background:#059669;color:white;text-decoration:none;padding:11px 18px;border-radius:999px;font-size:14px;font-weight:600;">Ver estado del pedido</a></p>`;
