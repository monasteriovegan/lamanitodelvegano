...
const QUANTITY_WORD = '(?:uno|una|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez|\\d+)';
const CART_ADD_INTENT = new RegExp(`(?:agrega|agregar|a[nñ]ade|a[nñ]adir|ponme(?:\\s+${QUANTITY_WORD})?|dame\\s+${QUANTITY_WORD}|quiero\\s+(?:comprar\\s+)?${QUANTITY_WORD}).{0,70}`, 'i');
