...
async function loadCatalog(db: SupabaseClient) {
  const { data } = await db
    .from('productos')
    .select('nombre,precio,disponibilidad,maneja_stock,stock,categoria')
    .eq('activo', true)
    .limit(40);
...
export async function maybeAutoReply(
...
): Promise<{ called: boolean; replied: boolean; reason?: string }> {
  if (persisted.duplicate || inbound.channel !== 'whatsapp' || inbound.direction !== 'inbound' || inbound.message_type !== 'text' || !inbound.text?.trim()) {
  ...
  const [{ data: config }, { data: conversation }] = await Promise.all([
    ...
    db.from('conversations')
      .select('id,ai_enabled,human_takeover,metadata,labels')
  ...
  if (conversation.metadata?.personal || conversation.labels?.includes?.('personal')) {
    return { called: false, replied: false, reason: 'personal_contact' };
  }

  const inboundAt = new Date(inbound.sent_at).getTime();
  if (!Number.isFinite(inboundAt) || Date.now() - inboundAt > SERVICE_WINDOW_MS) {
    return { called: false, replied: false, reason: 'service_window_closed' };
  }
  ...
  const history = (recent || []).reverse().map((message: any) => ({
    role: message.direction === 'outbound' ? 'model' : 'user',
    parts: [{ text: String(message.body || '') }],
