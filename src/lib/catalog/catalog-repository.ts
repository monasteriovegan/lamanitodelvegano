...
export function mapCatalogProductRow(businessUnitId: string, row: DbRow | null | undefined): CatalogProduct | null {
  ...
  return {
    ...
    sku: row.sku || null,
    glutenFree: row.gluten_free === null || row.gluten_free === undefined ? null : Boolean(row.gluten_free),
    nutFree: row.nut_free === null || row.nut_free === undefined ? null : Boolean(row.nut_free),
